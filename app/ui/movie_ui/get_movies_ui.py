...
from .update_movie_ui import UpdateMoviePage
...
class AllMoviesPage(QWidget):
    ...
    def open_update_page(self):
        ...
        if row == -1:
            QMessageBox.warning(self, "Ошибка", "Выберите фильм")
            return

        item = self.table.item(row, self.WATCHED_COL)
        movie_id = item.data(Qt.UserRole)

        self.update_window = UpdateMoviePage(self.stack, movie_id)
        self.update_window.destroyed.connect(self.load_movies)
